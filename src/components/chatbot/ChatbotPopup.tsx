...
const ChatbotPopup: React.FC<ChatbotPopupProps> = ({ showChat, setShowChat }) => {
    ...
    return (
        <div
            className="fixed bottom-20 right-5 bg-white rounded-lg shadow-2xl flex flex-col overflow-hidden z-50"
            style={{
                width: '450px',
                height: '500px',
        ...
        >
            ...
            <div className="p-4 border-t border-gray-200 bg-white flex items-center gap-2">
                <input
                    ...
                    className={`flex-grow p-3 rounded-full border ${isConnected ? 'border-gray-300' : 'border-gray-200 bg-gray-50'}
                                focus:outline-none focus:ring-2 focus:ring-green-500`}
                />
                <button
                    onClick={sendMessage}
                    disabled={!isConnected || inputMessage.trim() === ''}
                    className={`p-3 rounded-full ${isConnected && inputMessage.trim() !== '' ? 'bg-green-600 hover:bg-green-700' : 'bg-gray-300 cursor-not-allowed'}
